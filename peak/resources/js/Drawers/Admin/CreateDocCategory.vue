...
const store = () => {
  form.post(route('admin.docs.categories.store'), {
    preserveScroll: true,
    onSuccess: () => {
      form.reset();
      toast.success('Doc Category Created');
      open.value = false;
    },
    onError: (error) => {
      //
...
onMounted(() => {
  emitter.on('doc-category:create', (tag) => {
    open.value = true;
  });
});
                    ...
                    <div class="bg-blue-600 px-4 py-6 sm:px-6">
                      ...
                      <div class="mt-1">
                        <p class="text-sm text-blue-300">
                          Add a new category for documentation content.
                        </p>
                      </div>
                    </div>
