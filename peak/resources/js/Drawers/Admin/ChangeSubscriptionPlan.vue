...
const updatePlan = async () => {
  ...
  const confirmed = await useConfirm({
    ...
    text: __("Are you sure you want to change this user plan? This cannot be undone."),
  ...
  if (confirmed) {
    ...
    form.put(route('dashboard.account.billing.subscription.update'), {
      ...
      preserveScroll: true,
      onSuccess: () => {
        toast.success(__('Subscription plan changed'))
      },
      onFinish: () => {
        loader.hide();
      },
              ...
              <DialogPanel class="pointer-events-auto w-screen max-w-md">
                <form class="flex h-full flex-col divide-y divide-gray-200 bg-white shadow-xl"
                    ...
                    <div class="bg-blue-600 px-4 py-6 sm:px-6">
                      ...
                      <div class="mt-1">
                        <p class="text-sm text-blue-300">
                          {{ __('Change the user\'s current subscription plan.') }}
                        </p>
                      </div>
                    </div>
                    ...
                    <div class="flex flex-1 flex-col justify-between">
                      <div class="divide-y divide-gray-200 px-4 sm:px-6">
                        ...
                        <div class="space-y-6 pb-5 pt-6">
                          ...
                          <AlertInfo>
                            {{
                              __('Important: Plan change will happen immediately and depending on proration setting you set, user might be billed immediately full plan price or a proration is applied.')
                            }}
                          </AlertInfo>
                        </div>
                      </div>
                  ...
                  <div class="flex flex-shrink-0 justify-end px-4 py-4">
                    <SecondaryButton type="button" @click="open = false">{{ __('Cancel') }}</SecondaryButton>
                    <PrimaryButton
