...
const markAllAsRead = () => {
  ...
  router.patch(route('admin.notifications.read'), {}, {
    onSuccess: () => {
...
<template>
  <AdminLayout :description="__('Browse platform notifications')" :page-icon="false" :title="__('Notifications')">
    <template #actions>
    ...
    <div class="mb-4">
      <nav aria-label="Tabs" class="flex gap-x-4">
        <Link :class="{'bg-zinc-100 border border-zinc-200' : !isUnread}"
        ...
        <Link :class="{'bg-zinc-100 border border-zinc-200' : isUnread}"
              :href="route('admin.notifications.index', {type: 'unread'})"
    ...
    <div class="relative flex flex-col divide-gray-100 divide-y">
      <a v-for="notification in notifications" v-if="notifications.length > 0" :id="'notification-'+ notification.id"
         :key="notification.id"
         :href="route('admin.notifications.show', notification.id)"
         class="group relative rounded-md py-4 pl-6 pr-4 border-zinc-200 bg-white/50 hover:bg-gray-50">
      ...
      <div
          v-else
          class="w-full p-3 py-12 flex flex-col space-y-3 justify-center items-center bg-zinc-100 border border-zinc-200">
                <span>
                    <BellSlashIcon class="h-12 w-12 text-zinc-200"/>
                </span>
