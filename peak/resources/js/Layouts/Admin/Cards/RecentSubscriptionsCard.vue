...
<template>
  <Card :collapsible="collapsible" :deferred="lazy" :has-content-padding="false" :has-shadow="false"
        deferred-data="recentSubscriptions">
    <template #header>
      {{ __(title) }}
    </template>
    ...
    <ul v-if="recentSubscriptions.length > 0" class="divide-y divide-gray-100" role="list">
      <li v-for="subscription in recentSubscriptions"
          class="relative flex justify-between gap-x-6 px-4 py-5 hover:bg-gray-50">
        <div class="flex min-w-0 gap-x-4">
          <img :alt="subscription.user.name"
               :src="subscription.user.profile_photo_url"
               class="size-8 flex-none rounded-full bg-gray-50">
          <div class="min-w-0 flex-auto">
            <p class="text-sm/6 font-semibold text-gray-900">
              <Link
                  :href="route('admin.finance.subscriptions.edit', subscription.id)">
        ...
        <div class="flex shrink-0 items-center gap-x-4">
          <div class="hidden sm:flex sm:flex-col sm:items-end">
            <p class="text-sm/6 text-gray-900"><span class="font-semibold">
                            {{ subscription.plan_pricing.subscription_plan.name }}</span> /
              {{ subscription.plan_pricing.interval }}</p>
