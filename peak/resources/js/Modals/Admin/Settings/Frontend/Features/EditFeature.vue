...
onMounted(() => {
  emitter.on('feature:edit', function (featureContent) {
    editFeature.value = true;
    feature.value = featureContent;
    editFeatureForm.name = featureContent.name;
    editFeatureForm.description = featureContent.description;
    imagePreview.value = featureContent.image_url;
  });
});

const save = () => {
  editFeatureForm.post(route('admin.settings.frontend.features.update', {
    feature: feature.value?.id
  }), {
    onSuccess: () => {
      closeMe();
      toast.success(__('Feature item saved'));
    },
...
const deleteImage = () => {
  router.delete(route('admin.settings.frontend.features.image.delete', feature.value?.id), {
    preserveScroll: true,
    onSuccess: () => {
...
<template>
  <DialogModal :show="editFeature" max-width="xl" @close="closeMe">
    <template #title>
      {{ __('Edit Feature') }}
    </template>
    ...
    <template #content>
      <div class="flex flex-col space-y-3">
        <TextInput v-model="editFeatureForm.name" :placeholder="__('Question')"
                   class="w-full py-3"/>

        <InputError :message="editFeatureForm.errors?.name" class="mt-1"/>
        ...
        <InputError :message="editFeatureForm.errors?.description" class="mt-1"/>
          ...
          <div class="flex items-center justify-center w-full">
            <label
              ...
              <input id="main-logo" ref="imageInput" accept=".jpg, .jpeg, .gif, .png, .svg"
                     class="sr-only"
                     type="file"
                     @change="updateImagePreview" @input="editFeatureForm.image = $event.target.files[0]"/>
            </label>
          </div>

          <InputError :message="editFeatureForm.errors?.image" class="mt-1"/>
        </div>
    ...
    <template #footer>
      ...
      <PrimaryButton :class="{'opacity-25' : editFeatureForm.processing}" :disabled="editFeatureForm.processing"
                     class="ms-3"
                     @click="save">
        {{ __('Save Changes') }}
      </PrimaryButton>
