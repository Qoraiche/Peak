...
const addNewMenuItem = ref(false);
...
watch(externalLink, external => {
  if (external === true) {
    newMenuItem.reset('open_new_window', 'external_link');
  }
});

const toast = useToast();

const newMenuType = ref(null);

onMounted(() => {
  emitter.on('menu:add-item', function (menuType = 'header') {
    newMenuType.value = menuType;
    addNewMenuItem.value = true;
  });
});

const save = () => {
  newMenuItem.post(route('admin.settings.frontend.' + newMenuType.value + '.menu.add'), {
    onSuccess: () => {
      closeMe();
      toast.success(__('New Menu added'));
...
<template>
  <DialogModal :show="addNewMenuItem" max-width="md" @close="closeMe">
    <template #title>
