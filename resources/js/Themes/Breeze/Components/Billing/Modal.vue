<template>
    ...
    <transition enter-from-class="opacity-0" leave-active-class="opacity-0">
        <div v-if="visibleState > 0"
             class="fixed z-50 inset-0 bg-gray-800 bg-opacity-50 overflow-x-hidden overflow-y-auto transition-opacity duration-150 ease-linear"
             @click.self="_closeOutsideClick">
            <transition appear enter-from-class="-translate-y-5" enter-active-class="transform"
                        leave-active-class="transform" leave-to-class="-translate-y-5">
                <div v-if="visibleState > 1"
                     class="mb-32 mt-8 mx-auto pointer-events-none transition-transform duration-150 ease-out"
                     :style="{ maxWidth: maxWidth + 'px' }" :class="{ 'transform scale-105': bounce }">
                    <div ref="modal" class="modal p-6 bg-white rounded-xl shadow-lg pointer-events-auto">
                        <slot name="content" :close="close">
                            <div v-if="!!title || closeIcon" class="flex justify-between">
                                <slot name="close" :close="close">
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <button v-if="closeIcon" type="button" aria-label="Close"
                                                class="ml-2 text-gray-600 hover:text-gray-900 rounded-md"
                                                @click.prevent="close">
                                            clear
                                        </button>
                                    </div>
                            ...
                            <div class="mt-4">
                                <h4 v-if="loading" class="flex items-center justify-center my-10">
                                    {{ __('Loading') }}...</h4>
